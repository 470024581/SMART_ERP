...
function QueryForm() {
  ...
  const [activeDataSourceInfo, setActiveDataSourceInfo] = useState({ id: null, type: 'default', name: 'Default ERP' })
  ...
  const fetchActiveDataSource = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        setActiveDataSourceInfo({ id: 1, type: 'default', name: 'Default ERP' }) // Fallback hardcoded default
        return { id: 1, type: 'default' }
      }
    } catch (err) {
      console.error("Failed to fetch active data source type:", err)
      setActiveDataSourceInfo({ id: 1, type: 'default', name: 'Default ERP' }) // Fallback hardcoded default on error
      return { id: 1, type: 'default' }
    }
  }
